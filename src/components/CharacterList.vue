<template>
  ...
  }" class="mx-auto hb-card" :style="{ minHeight: '300px', width: '100%' }" @click="goToDetail">
    <v-badge v-if="isSelected" class="hb-card__badge" color="white" content="selected" inline></v-badge>
    <v-img :src="character.images.md" alt="Character Image" cover style="height: 200px; width: 100%;" />
...
const isSelected = computed(() =>
  selectedCharactersStore.selectedCharacters.some(
    (selectedCharacter: { id: number; }) => selectedCharacter && selectedCharacter.id === props.character.id
  )
);
